    ...
    <aside class="sidenav navbar navbar-vertical navbar-expand-xs border-0 border-radius-xl my-3 fixed-start ms-3   bg-gradient-dark" id="sidenav-main">
        ...
        <div class=" w-auto  max-height-vh-100" id="sidenav-collapse-main">
            <ul class="navbar-nav">
                ...
                <li class="nav-item">
                    <a class="nav-link text-white active bg-gradient-primary " href="../teacher/view_inserted_result.php">
                        <div class="text-white text-center me-2 d-flex align-items-center justify-content-center">
                            <i class="material-icons opacity-10">receipt_long</i>
                        </div>
...
// Define a flag for the update result
$update_success = null;
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!is_numeric($score) || $score < 0 || $score > 100) {
        $update_success = false; // Validation failed
    } else {
        // Update student result
        $result = $teacher->updateStudentResult($result_id, $score);
        if ($result) {
            $update_success = true; // Update was successful
        } else {
            $update_success = false; // Update failed
        }
    }
}

// Retrieve existing score for selected result
if (isset($_GET['id'])) {
    $result_id = $_GET['id'];
    $result = $teacher->getResult($result_id);
    if ($result) {
        $score = $result['score'];
    } else {
...
<main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg ">
            ...
            <div class="container-fluid py-1 px-3">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
                        ...
                        <li class="breadcrumb-item text-sm text-dark active" aria-current="page">Update Student Result</li>
                    </ol>
  ...
  <form method="POST" action="">
    ...
    <input type="hidden" name="result_id" value="<?php echo isset($result_id) ? $result_id : ''; ?>">
...
<?php if ($update_success === true) : ?>
    Swal.fire({
        ...
        if (result.isConfirmed) {
            window.location.href = 'View_inserted_result.php'; // Redirect to the desired page
        }
    });
